...
namespace WordleVariations
{
    ...
    {
        private static readonly string FIVE_LETTER_WORD_FILE_NAME = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), @"FiveLetterWords.txt");
        ...
        public static WordGetter CreateFromTextFile()
        {
            ...
            using(StreamReader reader = new StreamReader(FIVE_LETTER_WORD_FILE_NAME))
            {
                string line;
